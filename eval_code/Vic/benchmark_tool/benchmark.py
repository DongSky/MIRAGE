...
class visual_benchmark():
    ...
    def answer(self):
        return self.df['answer']

    def desp(self):
        return self.df['InternVL2-8B_describe']

    def init_thinking(self):
        return self.df['o3-mini_init_thinking']
    
    def refine_thinking(self):
        return self.df['refine_thinking']

    @property
    def left(self):
        mask = ~self.df.columns.isin(['image', 'decoded_image'])
    ...
    def __getitem__(self,index):
        ...
        if isinstance(index,int):
            ...
            # return {'prompt':self.prompt[index], 'image': self.df.iloc[index,self.df.columns.get_loc('image')],'answer':self.df.iloc[index,self.df.columns.get_loc('answer')],'desp':self.df.iloc[index,self.df.columns.get_loc('InternVL2-8B_describe')],'init_thinking':self.df.iloc[index,self.df.columns.get_loc('o3-mini_init_thinking')], 'classification': self.df.iloc[index,self.df.columns.get_loc('normalized_classification')], 'question_hint': self.df.iloc[index,self.df.columns.get_loc('question_specific_hint')]}   
            # return {'prompt':self.prompt[index], 'image': self.df.iloc[index,self.df.columns.get_loc('image')],'answer':self.df.iloc[index,self.df.columns.get_loc('answer')],'desp':self.df.iloc[index,self.df.columns.get_loc('InternVL2-8B_describe')],'init_thinking':self.df.iloc[index,self.df.columns.get_loc('o3-mini_init_thinking')], 'classification': self.df.iloc[index,self.df.columns.get_loc('normalized_classification')], 'question_hint': self.df.iloc[index,self.df.columns.get_loc('question_specific_hint')]} 
            # return {'prompt':self.df['prompt'][index], 'response': self.df[response_column][index], 'thinking':self.df.iloc[index,self.df.columns.get_loc('refine_thinking')]}  
            return {'prompt':self.df['prompt'][index], 'response': self.df[response_column][index], 'thinking':self.df.iloc[index,self.df.columns.get_loc('refine_thinking')], 'thinking_1':self.df.iloc[index,self.df.columns.get_loc('rephrase_thinking_1')], 'thinking_6':self.df.iloc[index,self.df.columns.get_loc('rephrase_thinking_6')]}   
    ...
    def generate_visual_chain_with_visual(self,row):
        ...
        prompt = row['prompt']
        image = [row['image']]
        if 'decoded_image' in row.index:
            image.append(row['decoded_image'])
