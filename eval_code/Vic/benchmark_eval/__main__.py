...
for file in file_list:
    match args.bench:
        ...
        case 'mathverse':
            evaluator = mathverse_eval.from_tsv(file)
        case 'mathvision':
            evaluator = mathvision_eval.from_tsv(file)
        case 'mmiq':
            evaluator = mmiq_eval.from_tsv(file)
        case 'mmvp':
        ...
        case 'mirage':
            evaluator = mirage_eval.from_tsv(file)
        case 'mirage_f1':
            evaluator = mirage_f1_eval.from_tsv(file)
    # evaluator.score()
    evaluator.eval()
